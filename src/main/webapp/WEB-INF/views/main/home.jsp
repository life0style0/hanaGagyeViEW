...
<link rel="stylesheet"
	href="/salmon/resources/template/style/jquery-ui.css">
<link rel="stylesheet"
	href="/salmon/resources/template/style/stylesheet.css">
...
<link rel="stylesheet"
	href="/salmon/resources/template/style/idangerous.swiper.css">
<link rel="stylesheet"
	href="/salmon/resources/jjw/css/stylesheet_jjw.css">
<link rel="stylesheet" href="/salmon/resources/jjw/css/common.css">
<link rel="stylesheet"
	href="/salmon/resources/jjw/css/stylesheet_jjw.css">
<link rel="stylesheet"
	href="/salmon/resources/jjw/css/owl.carousel.min.css">
<link rel="stylesheet"
	href="/salmon/resources/jjw/css/owl.theme.default.min.css">
...
<body>
								...
								<div class="row">
									<div class="col-md-6">
										<div class="custom-column-6">
											<div id="containerIncome"
												style="min-width: 30%; height: 400px; max-width: 70%; margin: 0 auto"></div>
										</div>
									</div>
									<div class="col-md-6">
										<div class="custom-column-4">
											<div id="containerSpend"
												style="min-width: 30%; height: 400px; max-width: 90%; margin: 0 auto"></div>
										</div>
									</div>
								</div>
							</div>
	...
	<div class="container-fluid cd-main-content custom-container"></div>
		...
		<div class="row">
			...
			<div class="col-md-2 left-feild">
				...
				<div class="be-user-detail">
					...
					<a href="/salmon/sns/feeds?userid=${me.user_id}">
				...
				<div class="be-vidget">
					<div class="creative_filds_block">
						<div class="controls">
							<button type="button" class="control btn btn-default my-btn"
								data-sort="likes-num:desc published-date:asc comments-num:asc">좋아요순</button>
							<button type="button" class="control btn btn-default my-btn"
								data-sort="comments-num:desc published-date:asc likes-num:asc">댓글순</button>
							<button type="button" class="control btn btn-default my-btn"
								data-sort="published-date:desc likes-num:asc comments-num:asc">최신순</button>
						</div>
					</div>
				</div>
				...
				<div class="be-vidget">
					<h3 class="letf-menu-article text-center feeds-smaller-title">소모임
						선택</h3>
					<div class="creative_filds_block">
						<div class="ul">
							...
							<c:choose>
								...
								<c:otherwise>
									<c:forEach var="group" items="${groupList}">
										<a data-filter=".group-<c:out value="${group.group_id}"/>"
											class="filter">${group.group_title}</a>
									</c:forEach>
								</c:otherwise>
							</c:choose>
						</div>
				...
				<div class="be-vidget">
					<h3 class="letf-menu-article text-center feeds-smaller-title">인기
						해시태그</h3>
						...
						<c:forEach var="article" items="${articleList }">
							...
							<c:choose>
								<c:when test="${article.group_id >0}">
									<div
										class="mix custom-column-3 ${tag} group-<c:out value="${group.group_id}"/>"
										data-published-date="${article.article_regdate}"
										data-likes-num="${article.likes.size()}"
										data-comments-num="${article.comments.size()}">
								</c:when>
								<c:otherwise>
									<div class="mix custom-column-3 ${tag} "
										data-published-date="${article.article_regdate}"
										data-likes-num="${article.likes.size()}"
										data-comments-num="${article.comments.size()}">
								</c:otherwise>
							...
							<div id="article-post-<c:out value="${article.article_id}"/>"
								class="be-post-sjh">
								<a class="be-img-block"> <c:if
										test="${article.imagePaths.size()==0 }">
								...
								<c:choose>
									<c:when test="${article.article_ctgry_id eq 3}">
										<%-- 역제안 게시글 --%>
										<a id="article-title-<c:out value="${article.article_id}"/>"
											class="be-post-title"> 역제안 </a>
									</c:when>
									<c:otherwise>
										<%-- 일반 게시글 --%>
										<a id="article-title-<c:out value="${article.article_id}"/>"
											class="be-post-title"> ${article.article_title} </a>
									</c:otherwise>
								</c:choose>
								...
								<div class="info-block info-block-sjh">
									<span><i class="far fa-thumbs-up"></i> <%--좋아요 --%> <c:out
											value="${article.likes.size()}" /> </span> <span><i
										class="fas fa-comment"></i> <%--댓글수 --%> <c:out
											value="${article.comments.size()}" /> </span>
								</div>
							</div>
				</div>
				</c:forEach>
	...
	<div class="theme-config">
		<div class="main-color">
			...
			<div class="colours-wrapper">
				<div class="entry color1 m-color active"
					data-colour="/salmon/resources/template/style/stylesheet.css"></div>
				<div class="entry color3 m-color"
					data-colour="/salmon/resources/template/style/style-green.css"></div>
				<div class="entry color6 m-color"
					data-colour="/salmon/resources/template/style/style-orange.css"></div>
				<div class="entry color8 m-color"
					data-colour="/salmon/resources/template/style/style-red.css"></div>
				<div class="title">Second Color:</div>
				<div class="entry s-color  active color10"
					data-colour="/salmon/resources/template/style/stylesheet.css"></div>
				<div class="entry s-color color11"
					data-colour="/salmon/resources/template/style/style-oranges.css"></div>
				<div class="entry s-color color12"
					data-colour="/salmon/resources/template/style/style-greens.css"></div>
				<div class="entry s-color color13"
					data-colour="/salmon/resources/template/style/style-reds.css"></div>
			</div>
		</div>
		<div class="open">
